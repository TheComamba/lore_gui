...
use crate::{
    dialog::{
        ...
        rename_descriptor::RenameDescriptorData, DialogMessage,
    },
...
impl SqlGui {
    pub(super) fn handle_message(&mut self, message: GuiMes) -> Result<(), LoreGuiError> {
        match message {
            ...
            GuiMes::DialogUpdate(event) => self.dialog.as_mut().unwrap().update(event),
            GuiMes::DialogSubmit => self.handle_message(self.dialog.as_ref().unwrap().submit())?,
